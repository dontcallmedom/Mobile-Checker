...
Checker.prototype.emulateBrowser = function(profile, url) {
    ...
    headless(opt, function(err, childProcess, servernum) {
        ...
        var browser = new Browser({
            ...
            // TODO: set proxyPort and avoid overlap
            browsermobProxy: {
                port: 8080
            },
            trackNetwork: true
        ...
        browser.on('fatalerror', function(err) {
            self.sink.emit('end');
            console.log(err.stack);
        });
...
Checker.prototype.report = function(key, name, category, status, data) {
    ...
    if (key == "ok") {
        ...
        this.formatReport(key, name, category, data, function (rep) {
            self.sink.emit('err', {
                "issue": rep,
                "status": status
            });
 ...
 * @param {Object} data optional data send with check result
 */
function reportFormatter(key, name, category, data, cb) {
    var str = fs.readFileSync(path.join(__dirname, '/reports/' + category + '/' +
        name + '/' + key + '.ejs'), 'utf8');
    data = data || {};
    data.name = name;
    data.category = category;
    formatter.format(ejs.render(str, data), category, cb);
}
/**
 * @augments Checker
 * @param {} sink events
