...
exports.check = function (checker) {
	...
	if (checker.reporting.analytics.speed <= 1000) {
		...
		sink.emit('warning', checker.l10n.message(checker.options.lang, this.category, this.name, "slow"));
		sink.emit('done');
	}
	else if (checker.reporting.analytics.speed > 2000) {
		sink.emit('err', checker.l10n.message(checker.options.lang, this.category, this.name, "veryslow"));
		sink.emit('done');
	}
}